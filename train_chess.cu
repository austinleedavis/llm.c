/*
Chess GPT-2 Transformer Neural Net training loop. See README.md for usage.
*/
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
typedef struct {
    int max_seq_len; // max sequence length, e.g. 512
...
void gpt2_build_from_checkpoint(GPT2 *model, const char* checkpoint_path, bool weight_init=true) {
    ...
    if (!(version == 3 || version == 5)) {
        ...
        fprintf(stderr, "---> HINT: try to re-run `python train_chess.py`\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (weight_init) {
        ...
        if (PRECISION_MODE == PRECISION_FP32 && version != 3) {
            ...
            fprintf(stderr, "---> HINT: to turn on FP32 you have to compile like: `make train_chesscu PRECISION=FP32`\n");
...
void gpt2_set_hyperparameters(GPT2Config* config, const char* depth_str) {
    ...
    else if (depth == 8)  { channels = 512; num_heads = 8; }
...
void gpt_build_from_descriptor(GPT2 *model, const char* descriptor) {
    ...
    // check the valid prefixes and dispatch to the right setup function
...
void error_usage() {
    fprintf(stderr, "Usage:   ./train_chesscu [options]\n");
    fprintf(stderr, "Options:\n");
    // file system input / output
    fprintf(stderr, "  -i <string> train data filename pattern (default = dev/data/tinyshakespeare/tiny_shakespeare_train.bin)\n");
    fprintf(stderr, "  -j <string> val data filename pattern (default = dev/data/tinyshakespeare/tiny_shakespeare_val.bin)\n");
    fprintf(stderr, "  -e <string> input .bin filename or descriptor, see code comments as docs. (default = chess_124M_bf16.bin)\n");
...
int main(int argc, char *argv[]) {
    ...
    const char* train_data_pattern = "dev/data/lichess_uci/00000000000000000010000/lichess_uci_train_*.bin";
    const char* val_data_pattern = "dev/data/lichess_uci/00000000000000000010000/lichess_uci_val_*.bin";
    const char* load_filename = "chess_124M_bf16.bin"; // bf16 weights of the model
    ...
    // from val instead of train split, because val is smaller and faster. (train_chess.py does the same)
    ...
    Tokenizer tokenizer;
    tokenizer_init(&tokenizer, "chess_tokenizer.bin");
