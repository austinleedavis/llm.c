"""
Reference code for Chess GPT-2 training and inference.
...
1 GPU:
python train_chess.py --write_tensors=0 --num_iterations=50 --sequence_length=5`1 --compile=1 --tensorcores=1 --dtype=bfloat16
you can also turn on flash-attention by appending --flash=1
4 GPU:
torchrun --standalone --nproc_per_node=4 train_chess.py --write_tensors=0 --num_iterations=50 --sequence_length=512 --compile=1 --tensorcores=1 --dtype=bfloat16
"""
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        assert model_type in {'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl', 'austindavis/chess-gpt2-uci-12x12x768', 'austindavis/chess-gpt2-uci-8x8x512'}
        from transformers import GPT2LMHeadModel
        print("loading weights from pretrained gpt: %s" % model_type)

        # n_layer, n_head and n_embd are determined from model_type
        config_args = {
            'austindavis/chess-gpt2-uci-8x8x512': dict(n_layer=8, n_head=8, n_embd=512),
            'austindavis/chess-gpt2-uci-12x12x768': dict(n_layer=12, n_head=12, n_embd=768), # 124M params
            'gpt2':         dict(n_layer=12, n_head=12, n_embd=768),  # 124M params
            'gpt2-medium':  dict(n_layer=24, n_head=16, n_embd=1024), # 350M params
            'gpt2-large':   dict(n_layer=36, n_head=20, n_embd=1280), # 774M params
            'gpt2-xl':      dict(n_layer=48, n_head=25, n_embd=1600), # 1558M params
        }[model_type]
        config_args['vocab_size'] = 72 # always 50257 for GPT model checkpoints
...
def _peek_data_shard(filename) -> int:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bin", type=str, default="dev/data/lichess_uci/00000000000000000010000/lichess_uci_train_*.bin", help="input .bin to train on")
    parser.add_argument("--input_val_bin", type=str, default="dev/data/lichess_uci/00000000000000000010000/lichess_uci_val_*.bin", help="input .bin to eval validation loss on")
    parser.add_argument("--output_dir", type=str, default="", help="output directory to which to write logs and checkpoints")
    parser.add_argument("--model", type=str, default="austindavis/chess-gpt2-uci-12x12x768", help="austindavis/chess-gpt2-uci-12x12x768|austindavis/chess-gpt2-uci-8x8x512|d8|d12")
    ...
    assert args.model in {"austindavis/chess-gpt2-uci-12x12x768","austindavis/chess-gpt2-uci-8x8x512","gpt2", "gpt2-medium", "gpt2-large", "gpt2-xl", "d12", "d24", "d36", "d48"}
    ...
    from dev.data.uci_tokenizers import UciTileTokenizer
    enc = UciTileTokenizer()
    if master_process and args.write_tensors: # tokenizer is technically not tensors but ok
        write_tokenizer(enc, "chess_tokenizer.bin")

    # init the model, either from scratch or from OpenAI pretrained checkpoint
    if args.model[0] == "d":
        # from scratch (random weights)
        model_config = {
            "d8": GPTConfig(block_size=512, vocab_size=72, n_layer=8, n_head=8, n_embd=512),
            "d12": GPTConfig(block_size=512, vocab_size=72, n_layer=12, n_head=12, n_embd=768),
        }[args.model]
    ...
    if master_process and args.write_tensors and (not args.inference_only):
        ...
        model_to_size = {"gpt2": "124M", "gpt2-medium": "355M", "gpt2-large": "774M", "gpt2-xl": "1558M", 'austindavis/chess-gpt2-uci-12x12x768':"124M"}
        model_to_size.update({f"d{d}": f"d{d}" for d in [12, 24, 36, 48]})
        model_size_str = model_to_size[args.model] # e.g. "124M", or "d12"
        write_model(model, f"chess_{model_size_str}.bin", dtype="float32")
        write_model(model, f"chess_{model_size_str}_bf16.bin", dtype="bfloat16")
        # save x, y, logits, loss, and parameter gradients, for debugging C
        # always store these in fp32 to have an accurate reference (?)
        write_state(model, x, y, logits, loss, f"chess_{model_size_str}_debug_state.bin")
