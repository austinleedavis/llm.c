"""
Lichess UCI dataset
https://huggingface.co/datasets/austindavis/lichess-uci
...
Example of downloading the 201301 dataset of Lichess UCI, from root directory:
...
parser.add_argument("--promotions_only", action="store_true", help="Ignore games without promotion; tokenize games with promotions (QBRN)", )
# fmt: on
...
if args.promotions_only:
    ...
    def filter_function(transcript):
        return (
            len(transcript) > 10  # not one of those '0-1' transcripts
            and len(transcript) % 5 != 4  # has promotion tokens
        )
    ...
    for tokens in pool.imap(tokenize, fw, chunksize=16):
        ...
        if token_count + len(tokens) < args.shard_size:
            ...
            filename = os.path.join(
                DATA_CACHE_DIR, f"{args.date}_{split}_{shard_index:06d}.bin"
            )
