...
loop of train_chess.cu. Compile:

make profile_chesscu NO_MULTI_GPU=1
...
sudo ncu --set full --import-source yes -o profile -f ./profile_chesscu
...
- `./profile_chesscu` is the executable we want to profile
...
int main(int argc, char *argv[]) {
    ...
    gpt2_build_from_checkpoint(&model, "chessGPT_d12_bf16.bin");

    int B = 16; // if program OOMs decrease this number, e.g. all the way down to 4 or etc
